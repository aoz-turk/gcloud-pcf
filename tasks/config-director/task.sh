...
if [[ $(echo "$deployed_products" | jq -r '.[] | select(.name=="p-bosh") | any') == "true" ]]
then
  echo "BOSH already deployed, skipping AZ and networks configuration"
else
  echo "BOSH not deployed, configuring AZs and networks"
